	...
	<head>
		...
		<script>
			...
			function concluida(id){
				location.href = 'todas_tarefas.php?acao=concluida&id=' + id
			}
		</script>
					...
					<div class="container pagina">
						<div class="row">
							<div class="col">
								...
								<?php foreach($tarefas as $key => $tarefa) { if($tarefa->status == 'pendente'){ ?>
									<div class="row mb-3 d-flex align-items-center tarefa">
										<div class="col-sm-9" id="tarefa_<?= $tarefa->id ?>"><?= $tarefa->tarefa; ?> (<?= $tarefa->status ?>)</div>
										<div class="col-sm-3 mt-2 d-flex justify-content-between">
											<i class="fas fa-trash-alt fa-lg text-danger" onclick="remover(<?= $tarefa->id ?>)"></i>
											<i class="fas fa-edit fa-lg text-info" onclick="editar(<?= $tarefa->id ?>, '<?= $tarefa->tarefa ?>')"></i>
											<i class="fas fa-check-square fa-lg text-success" onclick="concluida(<?= $tarefa->id ?>)"></i>
									</div>
									</div>
